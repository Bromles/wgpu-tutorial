...
* [Dawn](https://dawn.googlesource.com/dawn) - написан на C/C++, используется в Chromium. Использует Tint для
  компиляции шейдеров.
* [wgpu](https://wgpu.rs/) - написан на Rust, опционально поддерживает C, используется в Firefox. Использует Naga для
  компиляции шейдеров.
...
делать это так же просто, как на OpenGL или Metal, значительно проще Vulkan и DirectX 12.
...
* При использовании wgpu за пределами браузера, она предоставляет доступ к продвинутым возможностям современных нативных
  API за пределами стандарта WebGPU. Например, пуш константы, трассировка лучей, кеширование пайплайнов, рисование линий
  и другие.
...
* Развитая экосистема языка позволит использовать готовые решения типовых проблем. На момент написания, глобальный
  репозиторий библиотек языка Rust, [crates.io](https://crates.io/), содержит 170831 пакет. Для сравнения, conan
  (репозиторий библиотек на C/C++ от сообщества) содержит 1822 пакетов, а vcpkg (аналогичное решение от Microsoft) -
  2545\.
...
* [Стандарт WGSL](https://www.w3.org/TR/WGSL/) - Источник самой актуальной информации по языку шейдеров WebGPU. Следует
  помнить, что реализации, такие как Dawn и wgpu, не всегда поддерживают всё, что имеется в стандарте.
