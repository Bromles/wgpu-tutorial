...
Графическое API - это программный интерфейс, позволяющий управлять видеокартой из кода программ для отрисовки 2D или
...
* Отдельно можно выделить API в браузерах, например WebGL. Код фронтенда может работать с ними напрямую, но внутри
  самого
  браузера все равно используется библиотека-абстракция, совершающая вызовы нативного API текущей платформы. Таким
  образом, браузерные API можно тоже считать абстракциями над нативными.
...
* Vulkan - пришел на смену OpenGL, потенциально более производительный, более богатый по возможностям. Однако требует
  ...
  официальные инструменты отладки, необходимо полагаться на решения от сообщества. Последние годы стремительно теряет
  популярность.
...
среду, обеспечив полную кроссплатформерность и предоставив доступ к современному решению вместо устаревшей модели
WebGL/OpenGL.

Довольно скоро появилось две значимые реализации данного стандарта:

* [Dawn](https://dawn.googlesource.com/dawn) - написан на C/C++, используется в Chromium.
* [wgpu](https://wgpu.rs/) - написан на Rust с опциональной совместимостью с C, используется в Firefox.
...
Это дает возможность разрабатывать игры и другие графические приложения с современными фичами, но без
необходимости писать несколько реализаций под разные API и платформы, а также делать это так же просто, как на OpenGL
или Metal, значительно проще Vulkan и DirectX 12.
...
* Отсутствие проблем с кроссплатформерностью, подключением зависимостей и сборкой. Вместо 5 глав настройки проекта,
  ...
  и платформ-специфичного кода, а стандартная система сборки сама подключает зависимости и собирает проект, как в
  остальных современных языках.
...
* Множество корпораций уже переписали проекты с C/C++ и Go на Rust, а также правительство США рекомендует его в качестве
  ...
  новых разработчиков, а также занимает второе место по перспективности
...
* [webgpufundamentals](https://webgpufundamentals.org/) - руководство по использованию WebGPU в браузере на языке
...
* [Learn Wgpu](https://sotrh.github.io/learn-wgpu) - руководство по использованию wgpu на языке Rust, освещающее
  библиотеку и ее использование. Однако в нем используются неоптимальные библиотеки, и само руководство больше
...
* [Learn OpenGL](https://learnopengl.com/) - классическое, очень популярное руководство. Несмотря на то, что сам OpenGL
  ...
  CG вне зависимости от используемого API.
