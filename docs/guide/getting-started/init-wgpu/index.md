...
<p class="custom-block-title">Важное примечание</p>
...
Подведем итог, составив диаграмму последовательностей работы с нашим графическим API

```mermaid
sequenceDiagram
  participant App as App (winit)
  participant Surf as wgpu::Surface
  participant Dev as wgpu::Device
  participant Q as wgpu::Queue

  App->>Surf: get_current_texture()
  Surf-->>App: SurfaceTexture
  App->>Dev: create_command_encoder()
  App->>Dev: begin_render_pass(&view)
  App->>Dev: (set pipeline/bind groups, draw...)
  App->>Dev: end pass → finish() → CommandBuffer
  App->>Q: submit([CommandBuffer])
  App->>Surf: present()
```
