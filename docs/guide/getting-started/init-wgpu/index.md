...
На диаграмме выше wgpu указан в группе с Vulkan, DirectX 12 и Metal, поскольку на нативных платформах метод
`queue.submit` не блокирует CPU. Это отличается от стандарта WebGPU в браузере, где отправка команд
блокирует поток выполнения. Подробнее об этом рассказано ниже.

</div>

Данная схема позволяет добиться максимальной оптимизации. Однако она же значительно усложняет код и приводит к большому 
количеству ошибок, фактически требуя написания более высокоуровневых абстракций, чтобы избавиться от повсеместной 
...
Авторы стандарта WebGPU сознательно отказались от ручной синхронизации ресурсов - отправка команд на исполнение 
блокирует поток выполнения, пока они не будут исполнены. Нам теперь не нужно гадать, выполнились ли уже отправленные 
нами команды на GPU, как и не нужно писать отдельную синхронизацию ради этого.

Однако подход из стандарта лишает нас возможных оптимизаций, например подготовки ресурсов для рендера следующего кадра,
пока рендерится текущий. 

Поэтому wgpu, как реализация, пошла на нативных платформах тем же путем, что и Metal 3 - нам 
нет нужды заниматься явной ручной синхронизацией, как в Vulkan или DirectX, однако мы имеем возможность не 
блокировать CPU при отправке команд на GPU. Подробнее про то, как это возможно, будет рассказано далее в данной главе, 
на примере конкретного кода. Эти дополнительные оптимизации доступны лишь вне браузера, из-за ограничений его окружения.

То есть, wgpu позволяет нам не заниматься полностью ручной синхронизацией, но и дает простор для оптимизаций вне 
браузера, недоступный в самом стандарте. В отличие от нее, Dawn не дает ничего сверх стандарта, включая и подобные
возможности.

<div class="details custom-block">
<p class="custom-block-title">Есть и другие пути</p>

Авторы WebGPU придумали много более простых способов оптимизации, такие как `RenderBundle`, позволяющий
...
браузере (на момент написания — Safari на macOS 26+ и iOS 26+, Firefox на Windows или
...
Кроме того, мы можем симулировать логику игры для N+1 кадра, пока рендерим N-ый кадр, используя технику под названием
...
Если вам все еще кажется, что без полной ручной синхронизации будет слишком трудно добиться адекватной 
производительности игр, стоит помнить, что все игры и приложения, использующие графические API на системах Apple,
сделаны на Metal 3 или более ранних версиях — Metal 4 появился совсем недавно.

А в Metal 3 и ранее как раз автоматическая синхронизация ресурсов с опциональными оптимизациями, совсем как в wgpu. 
И при этом на платформах с ним выходят такие игры, как `Baldur's Gate 3`, `Death Stranding` и `Cyberpunk 2077`.
...
Кроме того, до сих пор множество игр делается на OpenGL, в котором не только нет синхронизации ресурсов, но и в
целом огромные участки API реализованы крайней неэффективно, намного хуже Metal 3 и wgpu. И этого тоже хватает для
реальных проектов.
...
Так мы здесь и делаем - задаем только поле `backends` равным `PRIMARY`, что означает поддержку Metal, Vulkan и DirectX 12.
То есть тех нижележащих API, которые позволяют использовать полные возможности wgpu, в отличие от вторичных бэкендов.
...
Вы могли слышать термин `Swapchain`. Он как раз относится к данному процессу.
В некоторых нативных API этот объект существует явно, как и в очень старых версиях wgpu. Но сейчас он сокрыт внутри
и не нуждается в ручном создании или управлении.

Когда мы запрашиваем `SurfaceTexture` у `Surface`, нам возвращается как раз текущая свободная текстура (если такая
есть), в которую мы можем производить отрисовку.
...
Здесь все просто - мы получаем новый размер поверхности как параметр, проводим защиту от нулевых значений, обновляем
...
Мы начинаем с того, что запрашиваем у поверхности текущую текстуру для отрисовки через `get_current_texture`. Этот
метод возвращает перечисление `CurrentSurfaceTexture`:

- `Success` — текстура получена успешно, можно отрисовывать
- `Suboptimal` — текстура получена, но состояние поверхности неоптимально. Мы всё равно можем рендерить,
  поэтому извлекаем текстуру и продолжаем как обычно
- `Outdated` — поверхность устарела, например после изменения размера окна. Переконфигурируем поверхность и
  пропускаем кадр
- `Lost` — поверхность потеряна. Аналогично переконфигурируем и пропускаем кадр
- `Timeout` — текстура ещё не готова. Просто пропускаем кадр, на следующем кадре попытка повторится
- `Occluded` — окно скрыто или полностью перекрыто другими окнами. Пропускаем отрисовку, чтобы не тратить
  ресурсы GPU впустую
- `Validation` — ошибка валидации, как правило свидетельствует об ошибке в коде. Логируем предупреждение
  и пропускаем кадр

<div class="info custom-block" style="padding-top: 8px">
<p class="custom-block-title">Примечание</p>

`Occluded` особенно полезен на мобильных платформах — когда приложение свёрнуто или перекрыто,
отрисовка кадров впустую расходует батарею. Поэтому мы не запрашиваем перерисовку в данных ветках,
а ждём очередного события от winit, которое триггернет `RedrawRequested`.
...
Можно считать это неким аналогом ссылки, понятной для GPU. Так как нам сейчас не нужно переопределять никакие параметры,
используем дескриптор по-умолчанию:
...
- `label` - отладочное название данной сущности
- `color_attachments` - массив сущностей, описывающих представления текстур, которые будут использоваться в качестве
  ...
  изображение.
    ...
    - `depth_slice` - слайс глубины для 3D текстур. На данный момент нами не используется, поэтому ставим `None`
...
- `queue.submit([encoder.finish()])` - мы завершаем запись команд в кодировщик, получая от него готовый закодированный
  `CommandBuffer`. И тут же передаем его в очередь на исполнение. Данный метод не ожидает выполнения команд на GPU и
  возвращает `SubmissionIndex`. Это позволяет готовить данные следующей отправки (например, кадра), пока текущая
  исполняется на видеокарте. Если же нам необходимо заблокировать поток на CPU и дождаться исполнения этих команд GPU
  (например, чтобы прочитать обратно какие-то данные из буферов), мы можем использовать `device.poll`, позволяющий 
  ждать как последней отправки, так и конкретной по `SubmissionIndex`.
- `window.pre_present_notify()` - уже упомянутое уведомление winit о том, что мы собираемся выводить кадр на экран.
- `frame.present()` - непосредственно отправка полученного кадра в виде текстуры в очередь отображения на экране.
  Этот метод тоже не дожидается вывода на экран в момент вызова, он лишь планирует его. Сам кадр для вывода
  же берется из предшествующего вызова `Queue.submit`, затрагивающего ту же текстуру, у которой вызывается `present`
  (в нашем случае - у `frame` был создан `view`, который использовался в `RenderPass`, который мы отправили на
  исполнение через `Queue.submit`. Наш `frame.present` видит данную связь и выводит отрендеренный кадр `frame`,
  когда он готов).
...
2. `Queue::submit()` крайне затратна для исполнения, поскольку включает в себя отслеживание ресурсов внутри
   отправленных буферов и их синхронизацию под капотом библиотеки. Однако данный метод принимает список
   `CommandBuffer`. Следовательно, разные части нашей программы должны создавать свои собственные буферы команд, и
   передавать их в один поток для разового исполнения, вместо множества вызовов метода `submit`.
