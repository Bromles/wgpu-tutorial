...
содержащего нашу* `main` *функцию:* `#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]`*. Это скажет
Rust, что мы делаем GUI приложение и не нуждаемся в консоли для его работы, но только в релизной сборке, так как во 
время разработки нам может понадобиться отладочный вывод.*
