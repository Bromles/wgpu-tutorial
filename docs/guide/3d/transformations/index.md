...
- векторы, матрицы, умножение матриц (глава «Векторы и матрицы»)
- путь вершины через системы координат (глава «Система координат WebGPU»)
...
ассоциативно, мы можем объединить все три в одну — `mvp` — и передать её в шейдер:
...
Bind group layout и bind group создаются так же, как в главе про uniform-буферы. Единственное отличие —
