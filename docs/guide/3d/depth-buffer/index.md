...
Формат `Depth32Float` — один `f32` на пиксель. Значения от 0.0 (ближняя плоскость) до 1.0 (дальняя). Флаг
`RENDER_ATTACHMENT` разрешает использовать текстуру как вложение depth/stencil в render pass.
...
`StoreOp::Store` сохраняет глубину после прохода. Обычно это не нужно (depth buffer используется только во время
рендера), но в некоторых случаях данные могут понадобиться позже — например, для shadow mapping.
