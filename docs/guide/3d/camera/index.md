...
- MVP-трансформации, view-матрица, `look_at_mat4`
...
До сих пор view-матрица была фиксированной — мы задавали её один раз через `look_at_mat4` и не меняли.
...
и `look_to_mat4` не может построить корректную матрицу.
...
`look_to_mat4` принимает позицию (`eye`), направление взгляда (`direction`) и вектор «вверх».
В отличие от `look_at_mat4` из предыдущих глав, здесь мы передаём направление, а не целевую точку.
...
В `render` view-матрица берётся из камеры вместо фиксированного `look_at_mat4`:

```rust
let view_mat = glam::camera::rh::view::look_at_mat4(Vec3::new(1.0, 1.5, 4.0), Vec3::ZERO, Vec3::Y);  // [!code --]
...
- Pitch ограничен ±89° — при ±90° вектор `direction()` становится параллелен `up` (Vec3::Y), и `look_to_mat4` не может построить корректную матрицу (вырожденное векторное произведение)
