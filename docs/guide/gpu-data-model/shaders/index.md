...
- язык шейдеров WGSL: типы, структуры, функции
...
возвращал константу, и GPU закрашивал весь треугольник одинаково. Не слишком впечатляюще. Хотелось бы задать каждой
вершине свой цвет и увидеть, как красный плавно перетекает в зелёный, зелёный в синий.

Для этого нужно передать данные о цвете с CPU на GPU через **вершинный буфер** — массив структур, описывающих каждую
вершину. А чтобы правильно описать эти структуры в шейдере, нам понадобится более глубокое понимание WGSL — языка,
на котором шейдеры написаны. Мы уже использовали его в прошлых главах, но не разбирали подробно. Пришло время это
исправить.

## WGSL: что нужно знать Rust-разработчику

WGSL (WebGPU Shading Language) — язык, на котором пишутся шейдеры. Его синтаксис во многом похож на Rust, и для
Rust-разработчика он ощущается знакомо. Но отличий достаточно, чтобы споткнуться: нет ownership и borrowing, нет
generics и traits, переменные объявляются через `var` вместо `let mut`, а `for` — C-style с инициализацией, условием
и шагом.

Разберём основы, достаточные для работы с шейдерами в этом руководстве.
...
let xyz = v.xyz;  // vec3<f32>(1.0, 3.0, 4.0) — нет, vec3<f32>(1.0, 2.0, 3.0)
```

Матрицы (`mat2x2<f32>`, `mat3x3<f32>`, `mat4x4<f32>`) и фиксированные массивы (`array<f32, 3>`) тоже есть, но они
нам понадобятся позже — в главе про трансформации.
...
Функции выглядят знакомо, но不支持 рекурсию и обобщённые типы (generics). Каждая функция вызывается с конкретными типами
аргументов:
...
Одна и та же структура может использоваться по-разному в зависимости от контекста — как вход или выход шейдера.
...
но ограничивало нас жёстко заданными координатами и одним цветом на весь треугольник. Научим шейдер принимать данные
извне — из вершинного буфера:
...
Разберём, как данные проходят через конвейер:
...
Интерполяция работает так: для каждого фрагмента GPU вычисляет, насколько близко он находится к каждой из вершин.
Фрагмент прямо по центру равноудалён от всех трёх вершин, поэтому его цвет — усреднённое значение: примерно
(0.33, 0.33, 0.33), то есть серый. Фрагмент рядом с красной вершиной будет преимущественно красным, с небольшой
примесью зелёного и синего.
...
Теперь создадим буфер и заполним его данными:
...
fn render(&mut self, _ctx: &GpuContext, view: &TextureView, encoder: &mut CommandEncoder) {
    let mut rpass = encoder.begin_render_pass(&RenderPassDescriptor {
        ...
        color_attachments: &[Some(RenderPassColorAttachment {
            ...
            resolve_target: None,
            ops: Operations {
                load: LoadOp::Clear(Color::GREEN),   // [!code --]
                load: LoadOp::Clear(Color::BLACK),   // [!code ++]
                store: StoreOp::Store,
            },
            depth_slice: None,
        })],
...
- `vec3<f32>` в WGSL struct выравнивается на 16 байт — занимает 16, а не 12. После `vec3` идёт 4 байта паддинга
