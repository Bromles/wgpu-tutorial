...
::: warning sRGB vs линейное пространство
Формат `Rgba8UnormSrgb` предназначен для цветовых текстур (diffuse/albedo) — GPU автоматически преобразует
sRGB→linear при сэмплировании и linear→sRGB при записи в surface. Для текстур с данными (normal maps, depth,
roughness) используйте `Rgba8Unorm` — он не выполняет преобразования цветового пространства. Частая ошибка —
использование `*Srgb` для normal maps: GPU применяет гамма-коррекцию к векторам нормалей, что приводит к неверному
освещению. HDR-форматы вроде `Rgba16Float` всегда хранятся в линейном пространстве и не имеют Srgb-варианта.
:::
...
| Размер uniform-буфера           | 16 КБ        | 256 МБ         |
