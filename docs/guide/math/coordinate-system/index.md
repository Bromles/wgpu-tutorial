...
**3. View → Clip** (directx::perspective с fov=45°, aspect=1, near=0.1, far=100):
...
::: warning Нулевая ближняя плоскость
Значение `near = 0` в `directx::perspective` приведёт к делению на ноль при перспективном делении. Ближняя плоскость отсечения всегда должна быть строго больше нуля — типичное значение `0.1`.
:::

::: warning Направление оси Y на экране
