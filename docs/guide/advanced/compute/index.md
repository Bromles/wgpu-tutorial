...
Обычные текстуры (`texture_2d`) доступны только для чтения в fragment-шейдерах.
Compute-шейдер записывает результат в **storage texture**:
...
GPU feature. Формат `rgba16float` совпадает с форматом текстуры — это обязательно.
